...
- (void)viewDidLoad {
    ...
    [webBtn setTitle:@"弹出允许WLAN权限对话框" forState:UIControlStateNormal];
    [webBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [webBtn addTarget:self action:@selector(gotoWebView) forControlEvents:UIControlEventTouchUpInside];
    webBtn.frame = CGRectMake(30, 150, 250, 30);
    [webBtn.layer setBorderWidth:2];
    [webBtn.layer setCornerRadius:10];
    [webBtn.layer setMasksToBounds:YES];
    webBtn.enabled=true;
