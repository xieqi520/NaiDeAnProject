...
@implementation SHIXGLController
...
- (void)viewDidLoad
{
    ...
    NSString *strResourcesBundle = [[NSBundle mainBundle] pathForResource:@"shixgl" ofType:@"bundle"];
    // 找到对应images夹下的图片
    //    NSString *strC = [[NSBundle bundleWithPath:strResourcesBundle] pathForResource:@”C” ofType:@”png” inDirectory:@”images”];
    
    
    //    NSURL *url =[NSURL fileURLWithPath:[[NSBundle bundleWithPath:strResourcesBundle]pathForResource:@"video1"ofType:@"mp4"]];
    
    
    // Create and compile fragment shader.
    fragShaderPathname = [[NSBundle bundleWithPath:strResourcesBundle] pathForResource:@"shix" ofType:@"f"];
    //NSLog(@"f路径: %@", fragShaderPathname);
    if(fragShaderPathname==nil){
        NSLog(@"路径错误，未读取到f");
    }
    // Create and compile vertex shader.
    vertShaderPathname = [[NSBundle bundleWithPath:strResourcesBundle] pathForResource:@"shix" ofType:@"v"];
    // NSLog(@"v路径: %@", vertShaderPathname);
...
- (BOOL)drawYUV
{
    ...
    if (m_pYUVData == NULL) {
        [m_YUVDataLock unlock];
         NSLog(@"drawYUV... m_nHeight   m_nWidth");
        return NO;
    }
